...
import { Search, Trash2, User, Calendar, MapPin, Package } from 'lucide-react';
import toast from 'react-hot-toast';
import { deleteItem, getAllItems, setItemApproval } from '../../modules/lostAndFound/lostAndFoundService';
...
const LostAndFoundManagement = () => {
  ...
  const handleApproval = async (itemId, nextApprovalStatus) => {
    try {
      const res = await setItemApproval(itemId, nextApprovalStatus);
      if (res?.success) toast.success(`Item ${nextApprovalStatus}`);
      loadItems();
    } catch (error) {
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className={`px-2 py-1 text-xs font-semibold rounded-full ${getApprovalColor(item.status === 'lost' ? (item.approvalStatus || 'approved') : 'approved')}`}>
                    {item.status === 'lost' ? (item.approvalStatus || 'approved') : 'approved'}
                  </span>
                </div>
              </div>
              ...
              <div className="flex justify-between items-center gap-3">
                <div className="flex gap-2">
                  {item.status === 'lost' && (item.approvalStatus || 'approved') === 'pending' ? (
                    <>
                      <button
                      ...
                      <button
                        ...
                        className="px-3 py-1 text-sm text-red-700 hover:text-red-900 hover:bg-red-50 rounded-md transition-colors"
                      >
                        Reject
                      </button>
                    </>
