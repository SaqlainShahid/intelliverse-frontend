...
export default function MessageBubble({ message, isOwn, onReply, onReact, onPin, canPin, canModerate, onEdit, onDelete, onVote }) {
  ...
  const ReactionsBar = () => {
    ...
    return (
      <div className={`mt-1 inline-flex items-center gap-1 rounded-full px-2 py-0.5 text-xs ${isOwn ? 'bg-green-700 text-white' : 'bg-gray-200 text-gray-800'}`}>
  ...
  const ReplyPreview = () => {
    ...
    return (
      <div className={`mb-1 text-xs rounded-md px-2 py-1 ${isOwn ? 'bg-green-700/40 text-white' : 'bg-gray-200 text-gray-800'}`}>
        <span className="opacity-80">Replying to: </span><span className="truncate">{text}</span>
      </div>
    );
  };
  ...
  return (
    <div id={`message-${message._id}`} className={`flex ${isOwn ? 'justify-end' : 'justify-start'} mb-1.5 px-2`}>
      <div className={`group max-w-[68%] rounded-2xl px-3 py-2 shadow-sm ${isOwn ? 'bg-green-600 text-white rounded-tr-sm' : 'bg-gray-100 text-gray-900 rounded-tl-sm'}`}>
        <ReplyPreview />
        {message.isDeleted ? (
        ...
        <div className={`mt-1 flex items-center gap-2 ${isOwn ? 'justify-end' : 'justify-start'}`}>
          <button onClick={onReply} className={`text-[11px] inline-flex items-center gap-1 ${isOwn ? 'text-green-100' : 'text-gray-600'}`} title="Reply">
            <Reply className="h-3.5 w-3.5" /> Reply
          </button>
          <button onClick={() => onReact && onReact('👍')} className={`text-[11px] inline-flex items-center gap-1 ${isOwn ? 'text-green-100' : 'text-gray-600'}`} title="React">
            <Smile className="h-3.5 w-3.5" /> React
          </button>
          {canPin && (
            <button onClick={onPin} className={`text-[11px] inline-flex items-center gap-1 ${isOwn ? 'text-green-100' : 'text-gray-600'}`} title="Pin/Unpin">
              <Pin className="h-3.5 w-3.5" /> Pin
            </button>
          )}
          {(isOwn || canModerate) && !message.isDeleted && message.type !== 'poll' && (
            <>
              <button type="button" onClick={() => setEditing(true)} className={`text-[11px] inline-flex items-center gap-1 ${isOwn ? 'text-green-100' : 'text-gray-600'}`} title="Edit">
                <Edit3 className="h-3.5 w-3.5" /> Edit
              </button>
              <button type="button" onClick={() => onDelete && onDelete()} className={`text-[11px] inline-flex items-center gap-1 ${isOwn ? 'text-green-100' : 'text-gray-600'}`} title="Delete">
                <Trash2 className="h-3.5 w-3.5" /> Delete
              </button>
            </>
          )}
        </div>
        <div className={`mt-1 flex items-center gap-1 ${isOwn ? 'justify-end text-green-100' : 'justify-end text-gray-500'}`}>
